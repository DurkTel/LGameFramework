...
using System.IO;
using System;
using static LGBuild;
using System.Linq;
using UnityEngine.Networking;

public class LGBuild : EditorWindow
{
    ...
    {
        LGBuild resetPivot = (LGBuild)EditorWindow.GetWindow(typeof(LGBuild));
    ...
    {
        string project = LGBuildUtility.GetProjectPath(AssetDefine.s_LuaBuildTemp);
        //先清空临时文件夹
    ...
    private static void CollectionAudio(ref List<AssetBundleBuild> list, string audioFolder)
    {
        ...
        foreach (var folder in allFolder)
        {
            ...
            {
                LGBuildUtility.CollectionFolder(list, son);
            }
        }
        ...
        {
            LGBuildUtility.CollectionFile(list, file);
        }
    }
    ...
    private static void CollectionEffect(ref List<AssetBundleBuild> list, string effectFolder)
    {
        ...
        foreach (var folder in allFolder)
        {
            LGBuildUtility.CollectionFolder(list, folder);
        }
    }
    ...
    private static void CollectionFont(ref List<AssetBundleBuild> list, string fontFolder)
    {
        ...
        LGBuildUtility.CollectionFolder(list, fontFolder);
    }
        ...
        foreach (var folder in allFolder)
        {
            ...
            if (Directory.Exists(imagesFolder))
            {
                ...
                if (topImgs.Length > 0)
                {
                    ...
                    list.Add(LGBuildUtility.CreateAssetBundleBuild(bundlName, addressableNames, assetNames));
                }
                ...
                foreach (var childFolder in childFolders)
                {
                    ...
                    {
                        LGBuildUtility.CollectionFolder(list, childFolder, true);
                    }
                    //其他文件夹 一个文件夹一个包
                    else if (childImgs.Length > 0)
                    {
                        LGBuildUtility.CollectionFolder(list, childFolder);
                    }
                }
            }
            ...
            {
                LGBuildUtility.CollectionFolder(list, prefabsFolder, true);
            }
        }
    ...
    { 
        ...
        foreach (string dll in dllNames)
        {
            ...
            if (File.Exists(dllPath))
            {
                ...
                string targetPath = LGBuildUtility.GetFullPath(projectPath);
    ...
    {
        RefreshAssetsBundleManifest(Path.Combine(buildParameter.buildOutPath, buildParameter.buildTarget.ToString()));
        ...
        foreach (var childDir in allDir)
        {
            ...
            foreach (var abPath in allAB)
            {
                ...
                foreach (var assetName in allAsset)
                {
                    ...
                    string md5 = LGBuildUtility.GetMD5(Path.Combine(path, newABName));
